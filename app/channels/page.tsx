...
function tokenHealth(channel: Channel): 'good' | 'expiring' | 'expired' | 'unknown' {
    if (!channel.created_at) return 'unknown';
    const daysSince = (Date.now() - new Date(channel.created_at).getTime()) / (1000 * 60 * 60 * 24);
    if (daysSince > 55) return 'expired';
    if (daysSince > 40) return 'expiring';
    return 'good';
}
                            ...
                            <IOSCard key={channel.id} className="p-4 group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[12px] text-ios-text-secondary font-mono mt-0.5 truncate">ID: {channel.account_id}</p>
