...
export async function createVideoThumbnailFile(file: File): Promise<File | null> {
    ...
    try {
        ...
        const blob = await withTimeout(
            new Promise<Blob | null>(resolve => canvas.toBlob(resolve, 'image/webp', 0.82)),
            5_000,
            'thumbnail encode',
        );
        if (!blob) return null;
        ...
        console.warn('Video thumbnail generation failed (skipping):', error instanceof Error ? error.message : error);
        return null;
    } finally {
