 ...
 */
import { Prisma, type ContentItem, type Planner, type Post } from '@prisma/client';
import {
...
type PrismaLike = {
    contentItem: {
        findFirst: (args: Prisma.ContentItemFindFirstArgs) => Promise<ContentItem | null>;
        findMany: (args: Prisma.ContentItemFindManyArgs) => Promise<ContentItem[]>;
    };
    planner: {
        updateMany: (args: Prisma.PlannerUpdateManyArgs) => Promise<Prisma.BatchPayload>;
...
function selectContentIndex(contentList: PlannerContentItem[], sortOrder: string, state: Record<string, any>) {
    ...
    if (sortOrder === 'random_loop') {
        ...
        const last = nextState.last_index !== undefined ? Number(nextState.last_index) : contentList.length;
        const clamped = Math.min(Number.isFinite(last) ? last : contentList.length, contentList.length);
        ...
        const last = nextState.last_index !== undefined ? Number(nextState.last_index) : -1;
...
async function applyCaptionTemplate(opts: {
    ...
    const templates = Array.isArray(opts.config.caption_templates)
        ? opts.config.caption_templates.filter((t: unknown) => typeof t === 'string' && t.trim().length > 0)
        : [];
    ...
    const chosen = rotation === 'random'
        ? templates[Math.floor(Math.random() * templates.length)]
        : templates[(opts.templateIndex + opts.postOrdinal) % templates.length];

    return chosen.replace(/\{post_title\}|\{post_caption\}|\{date\}|\{channel_name\}|\{hashtags\}/g, (m: string) => vars[m] ?? '');
}
...
export async function buildPostData(opts: {
    ...
    const isVideoStory = runtime.mediaType === 'STORIES' && !!runtime.mediaUrl && runtime.mediaUrl.includes('.mp4');
    ...
    return {        user_id: opts.planner.user_id,
        channel_id: opts.channel.id,
        status: 'pending',
        media_type: runtime.mediaType,
        video_url: (runtime.mediaType === 'REELS' || isVideoStory) ? runtime.mediaUrl : null,
        image_url: (runtime.mediaType === 'IMAGE') ? runtime.mediaUrl
            : (runtime.mediaType === 'STORIES' && runtime.mediaUrl && !runtime.mediaUrl.includes('.mp4')) ? runtime.mediaUrl
            : (runtime.mediaType === 'CAROUSEL' && safeChildren.length > 0) ? safeChildren[0].url
            : null,
        thumbnail_url: runtime.thumbnailUrl || (safeChildren.length > 0 ? safeChildren[0].url : null),
        ...
        audio_configuration: runtime.audioConfiguration ? JSON.stringify(runtime.audioConfiguration) : null,
        caption,
...
export async function resolvePlannerRuntime(prisma: PrismaLike, planner: any, now = new Date()) {
    ...
    const libId = selectedContent.id || selectedContent.folder_id;
    const isLibraryEntry = selectedContent.type === 'library_item'
    ...
    if (isLibraryEntry && libId) {
        ...
        const libItem = await prisma.contentItem.findFirst({ where: { id: libId, user_id: planner.user_id } });
        if (libItem) {
...
export async function runPlannerOnce(
    ...
    now = new Date(),
    opts?: { force?: boolean }
): Promise<{ ok: boolean; created?: number; skipped?: string; error?: string; warnings?: string[] }> {
    const config = parsePlannerConfig(planner.config);

    const validation = validatePlannerConfig(config);
    if (!validation.ok) {
        return { ok: false, skipped: 'invalid_config', error: `Config inválido: ${validation.errors.join('; ')}` };
    }
    ...
    if (!opts?.force) {
        ...
        if (intervalMs === null) {
            return { ok: false, skipped: 'invalid_frequency', error: 'frequency.value inválido (deve ser >= 1)' };
        }
        ...
        if (config.start_time && now < new Date(String(config.start_time))) {
            return { ok: false, skipped: 'start_time', error: 'start_time not reached' };
        }
    ...
    if (channels.length === 0) {
        return { ok: false, skipped: 'no_channels', error: 'No channels connected' };
    }

    const publishableChannels = channels.filter((channel: any) => describeChannelHealth(channel, now).ok);
    const blocked = channels.length - publishableChannels.length;
    if (publishableChannels.length === 0) {
        return { ok: false, skipped: 'no_publishable_channels', error: 'No publishable channels available' };
    }
    ...
    if (!runtime.ok) {
        return { ok: false, skipped: 'resolution_failed', error: runtime.errors.join('; '), warnings: runtime.warnings };
    }
    ...
    if (claim.count !== 1) {
        return { ok: false, skipped: 'already_running', error: 'Planner is already running (claim lost)' };
    }

    // ── Templates: índice base do state, incrementado POR POST criado ─────────
    const state = parsePlannerState(planner.state);
    const templateIndex = typeof state.template_index === 'number' && Number.isFinite(state.template_index)
        ? state.template_index
        : 0;
    const rotation = config.caption_rotation || 'off';
    const useTemplates = Array.isArray(config.caption_templates) && config.caption_templates.length > 0 && rotation !== 'off';
    ...
    for (let i = 0; i < publishableChannels.length; i++) {
        postDatas.push(await buildPostData({
            prisma,
    ...
    ops.push(prisma.planner.update({
        where: { id: planner.id },
    ...
    return {
        ...
        warnings: [...(runtime.warnings || []), ...(blocked > 0 ? [`${blocked} channel(s) blocked`] : [])],
    };
}
