#!/usr/bin/env bash
#
# Calendar gauntlet driver — boots the app (prod standalone or dev fallback)
# and runs:
#   1. calendar-visual.mjs   (C4-render, C5-modal, C7, C8 — Playwright, browser TZ=America/Sao_Paulo)
#   2. calendar-scenarios.mjs (C1-C6 — invariant harness over real HTTP)
#   3. calendar-perf.mjs     (C9 — 300-post perf baseline)
#
...
#                        (default: <repo>/gauntlet-runs/module-05-calendar).
#
# Exit 0 only if ALL parts pass. Temp DB/uploads die with the run.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
RUN_DIR="${RUN_DIR:-$REPO_ROOT/gauntlet-runs/module-05-calendar}"
MODE="${MODE:-prod}"
OUT_DIR="$RUN_DIR/out"

TMP="$(mktemp -d "${TMPDIR:-/tmp}/calendar-gauntlet.XXXXXX")"
mkdir -p "$TMP/out"
...
TS="$(date +%H%M%S)"
{
	echo "# Calendar baseline — $(date -u +%Y-%m-%dT%H:%M:%SZ)"
	echo
	echo "Mode: $MODE | Commit: $(git -C "$REPO_ROOT" rev-parse --short HEAD)"
	echo "Evidence dir: $TMP"
	echo
	echo "## C1-C6 (invariant harness)"
	echo
	echo '```'
	cat "$TMP/out/scenarios.txt"
	echo '```'
	echo
	echo "## C7/C8 (visual — console errors + TZ placement)"
	echo
	echo '```'
	cat "$TMP/out/visual.txt"
	echo '```'
	echo
	echo "## C9 (perf baseline)"
	echo
	echo '```'
...
mkdir -p "$OUT_DIR"
cp "$TMP"/out/calendar-*.png "$OUT_DIR/" 2>/dev/null || true

echo
if [ "$RC_ALL" -eq 0 ]; then
	echo "==> ALL CALENDAR SCENARIOS PASS"
else
	echo "==> CALENDAR SCENARIOS FAILED (exit $RC_ALL)"
fi
exit "$RC_ALL"
